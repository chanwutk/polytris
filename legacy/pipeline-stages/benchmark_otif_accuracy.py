...
def main():
    ...
    files = []
    for file in os.listdir('./tracking_results_otif'):
        if file.startswith('tracked'):
            files.append(file)
    
    with mp.Pool(processes=mp.cpu_count()) as pool:
        [*pool.starmap(run_benchmark, [(dataset_config, eval_config, metrics_config, os.path.join('./tracking_results_otif', file)) for file in files])]
        
            # run_benchmark(dataset_config, eval_config, metrics_config, os.path.join('./tracking_results_otif', file))


def run_benchmark(dataset_config: dict, eval_config: dict, metrics_config: dict, file):
    matrices = [trackeval.metrics.HOTA]

    gt_file = file.replace('tracked_op_', 'tracked_no_op_')
    if '_isr_' in gt_file:
        gt_file = gt_file.split('_isr_')[0] + '.jsonl'
