#!/usr/bin/env python3
"""
YOLOv5 Speed Test Experiment

This script tests the inference speed of YOLOv5 on different video resolutions:
- 480p (854x480)
- 720p (1280x720) 
- 1080p (1920x1080)
...
def load_yolov5_model():
    ...
    try:
        # Load YOLOv5 model
        model = YOLO('yolov5s.pt')  # Using YOLOv5s for speed comparison
        print("Loaded YOLOv5s model successfully")
        return model
...
def main():
    parser = argparse.ArgumentParser(description='Test YOLOv5 inference speed on different resolutions')
    ...
    for resolution_name, target_resolution in RESOLUTIONS.items():
        ...
        if speed_results:
            ...
            # Save results
            results_file = output_dir / f"yolov5_{resolution_name}_speed_results.json"
        ...
        if args.visualize:
            viz_output = output_dir / f"yolov5_{resolution_name}_visualization.mp4"
            create_visualization(args.video, model, resolution_name, target_resolution, str(viz_output))
    
    # Save combined results
    combined_results_file = output_dir / "yolov5_all_resolutions_results.json"
