...
def load_pruned_classification_results(dataset: str, video_file: str,
    ...
    Args:
        ...
        relevance_threshold (float): Relevance classifier binarization threshold (0–1)
        sample_rate (int): Sample rate for frame sampling (default: 1, process all frames)
        verbose (bool): Whether to print verbose output
...
def format_relevance_threshold_token(threshold: float) -> str:
    # Encode relevance threshold T_r as r### (percent) so it cannot collide with tracking_accuracy tokens.
    return f'r{int(round(threshold * 100)):03d}'
...
def build_param_str(*, classifier: str, tilesize: int,
                    ...
                    relevance_threshold: float | None = None) -> str:
    ...
    if relevance_threshold is not None:
        parts.append(format_relevance_threshold_token(relevance_threshold))
...
def parse_execution_param_str(param_str: str) -> dict[str, typing.Any]:
    ...
    # Parse optional relevance threshold token (r###, percent) when present.
    if (cursor < len(parts) and len(parts[cursor]) == 4 and parts[cursor].startswith('r')
            and parts[cursor][1:].isdigit()):
        relevance_threshold = int(parts[cursor][1:]) / 100.0
        cursor += 1
