...
# sys.path.append(str(MODULES_PATH / 'transformers' / 'src'))
...
PIPELINE_DIR = 'pipeline-transformers'
...
def main(args):
    ...
    with torch.no_grad():
        fpd = open(os.path.join(TRACK_RESULTS_DIR, f'{videofilename}.d.jsonl'), 'w')
        fpr = open(os.path.join(TRACK_RESULTS_DIR, f'{videofilename}.r.jsonl'), 'w')
        fpp = open(os.path.join(TRACK_RESULTS_DIR, f'{videofilename}.p.jsonl'), 'w')
        try:
...
if __name__ == '__main__':
    ...
    try:
        for idx, videofile in enumerate(os.listdir(VIDEO_DIR)):
            ...
            input = Input(os.path.join(VIDEO_DIR, videofile), str(idx % torch.cuda.device_count()))
            # main(input)
