...
1. Number of compressed images for each (dataset, classifier, tilesize, sample_rate, tilepadding, scale, stage)
...
def parse_config_names(df: pd.DataFrame) -> pd.DataFrame:
    """
    Parse configuration names to extract classifier, tilesize, sample_rate, tilepadding, and scale.

    Args:
        df: DataFrame with config_name column

    Returns:
        DataFrame with added columns: classifier, tilesize, sample_rate, tilepadding, scale
    """
    # Split config names using pandas string operations
    # Format: classifier_tilesize_samplerate_tilepadding_s{pct}
    config_parts = df['config_name'].str.split('_', expand=True)
    config_parts.columns = ['classifier', 'tilesize', 'sample_rate', 'tilepadding', 'scale']

    # Convert tilesize and sample_rate to int
    config_parts['tilesize'] = config_parts['tilesize'].astype(int)
    config_parts['sample_rate'] = config_parts['sample_rate'].astype(int)
...
def compute_metrics(df: pd.DataFrame, verbose: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    id_cols = ['dataset', 'video', 'stage', 'classifier', 'tilesize', 'sample_rate', 'tilepadding', 'scale']
...
def save_comparison_tables(df: pd.DataFrame, runtime_df: pd.DataFrame, verbose: bool = False):
    ...
    for dataset, group_df in df.groupby('dataset'):
        ...
        image_cols = ['dataset', 'video', 'stage', 'classifier', 'tilesize', 'sample_rate', 'tilepadding', 'scale', 'num_images', 'total_frames']
