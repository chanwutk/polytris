...
def save_tex_macros(summary_df: pd.DataFrame, output_path: str) -> None:
    ...
    # Resolve the 5% summary row used for the prior-failure placeholder.
    five_pct_row = summary_df.loc[summary_df['threshold'] == 0.05].iloc[0]
    # Resolve the 10% summary row used for the throughput range placeholder.
    ten_pct_row = summary_df.loc[summary_df['threshold'] == 0.10].iloc[0]
    ...
    with open(output_path, 'w') as f:
        ...
        # Write the 5% prior-system failure-count placeholder.
        f.write(
        ...
        f.write(
            ...
            f'\\autogen{{{float(ten_pct_row["speedup_min_x"]):.2f}}}}}\n'
        )
        # Write the 10% maximum speedup placeholder.
        f.write(
