...
def prepare_throughput(throughput: pd.DataFrame) -> pd.DataFrame:
    ...
    # Group by video, classifier, tilesize, tilepadding, sample_rate, and tracker, then sum the times
    cols = ['video', 'classifier', 'tilesize', 'tilepadding', 'sample_rate', 'tracker']
...
def match_accuracy_throughput_data(
    ...
    """
    Match accuracy and throughput data by video/classifier/tilesize/tilepadding/sample_rate/tracker combination.
    ...
    for _, row in tradeoff_combined.iterrows():
        ...
        print(f"Using actual combined accuracy scores for {row['classifier']}_{row['tilesize']}_{row['tilepadding']}_SR{row['sample_rate']}{tracker_str}: " \
...
def main():
    ...
    4. Matches the data by video/classifier/tilesize/tilepadding/sample_rate/tracker combination
