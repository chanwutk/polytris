...
```bash
ssh ace 'docker exec polyis bash -c "cd /polyis && python viz/operator-animation/generate_data.py --dataset jnc0 --video te01.mp4 --frame-start 704 --overwrite"'
```
...
[done] Wrote data to /polyis/viz/operator-animation/data
```
...
| `--output-dir` | `viz/operator-animation/data` | Where to write outputs |
...
```bash
ssh ace 'docker exec polyis bash -c "cd /polyis && python viz/operator-animation/generate_data.py --video te01.mp4 --frame-start 0 --overwrite"'
```

Example — finer M slider (21 values, slower):

```bash
ssh ace 'docker exec polyis bash -c "cd /polyis && python viz/operator-animation/generate_data.py --frame-start 192 --m-values 0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.0 --overwrite"'
```
...
```bash
ssh ace 'docker exec polyis bash -c "cd /polyis && python viz/operator-animation/scan_activity.py"'
```
...
```bash
rsync -av --delete ace:/work/cwkt/projects/polyis/viz/operator-animation/data/ viz/operator-animation/data/
```
...
| `data/` exists, script refuses to run | Pass `--overwrite`, or `rm -rf viz/operator-animation/data` first. |
...
./sync && \
  ssh ace 'docker exec polyis bash -c "cd /polyis && python viz/operator-animation/generate_data.py --dataset jnc0 --video te01.mp4 --frame-start 704 --overwrite"' && \
  rsync -av --delete ace:/work/cwkt/projects/polyis/viz/operator-animation/data/ viz/operator-animation/data/ && \
  (cd viz/operator-animation && python3 -m http.server 8765)
```
