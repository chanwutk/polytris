...
```bash
./sync
ssh ace 'cd <repo_path> && docker exec polyis ./run viz/operator-animation/generate_data.py'
rsync -av --delete ace:<repo_path>/viz/operator-animation/data/ viz/operator-animation/data/
```
...
```bash
./run viz/operator-animation/generate_data.py --dataset jnc0 --video te01.mp4 --frame-start 704 --overwrite
```
...
Serve the `viz/operator-animation/` directory and open `index.html`:

```bash
cd viz/operator-animation && python -m http.server 8000
